...
async def get_user_id_from_token(authorization: str) -> str:
    """Extract profile user_id from JWT token"""
    ...
    # CognitoAuth.verify_token returns 'id' as the cognito user id
    cognito_id = payload.get("id") or payload.get("sub")
    if not cognito_id:
        raise HTTPException(status_code=401, detail="Invalid token - no id claim")

    # Convert Cognito ID to profile UUID (not async)
