...
import {
  Mail, Plus, Search, Archive, Inbox, RefreshCw, ArrowLeft, Reply, ReplyAll,
...
import {
  ...
  useArchiveThread, useStarThread, useBulkThreadAction, useEmailLabels,
...
function buildQuotedReply(messages: any[], threadSubject: string): { quotedHtml: string; quotedLabel: string } | null {
  ...
  const dateStr = format(new Date(lastMsg.created_at), 'MMM d, yyyy h:mm a');
...
const CRMEmail = () => {
  ...
  const handleForward = (msg: any) => {
    const fwdBody = `<br><br><div style="border-left: 2px solid #ccc; padding-left: 12px; margin-left: 8px; color: #888;">` +
      ...
      `<p>From: ${msg.from_address}<br>Date: ${format(new Date(msg.created_at), 'MMM d, yyyy h:mm a')}<br>Subject: ${msg.subject || thread?.subject || ''}</p>` +
                  ...
                  {scheduledMessages.length === 0 ? (
                    ...
                    <div key={msg.id} className="px-4 py-3 hover:bg-muted-gray/20">
                      ...
                      <p className="text-xs text-accent-yellow mt-1">
                        ...
                        Scheduled for {format(new Date(msg.scheduled_at), 'MMM d, h:mm a')}
                      </p>
                    </div>
                  ))}
                </div>
