...
serve(async (req) => {
  ...
  try {
    ...
    const validStatuses = ['pending', 'approved', 'rejected', 'archived', 'in review', 'considered'];
    if (!submissionId || !status || !validStatuses.includes(status)) {
      return new Response(JSON.stringify({ error: 'Invalid request: submissionId and a valid status are required.' }), {
        status: 400,
        headers: { ...corsHeaders, 'Content-Type': 'application/json' },
      })
    }
    ...
    // 4. Update the submission's status
    ...
    if (updateError) {
      console.error('Supabase submission update error:', updateError);
      throw updateError
    }

    // 5. Return success
    return new Response(JSON.stringify({ message: `Submission status updated to ${status}` }), {
      headers: { ...corsHeaders, 'Content-Type': 'application/json' },
      status: 200,
    })

  } catch (error) {
    console.error('Caught unhandled error in update-submission-status function:', error);
