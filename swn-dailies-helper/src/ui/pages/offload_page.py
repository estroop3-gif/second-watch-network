...
class OffloadPage(QWidget):
    ...
    def _trigger_proxy_generation(self):
        """Trigger proxy generation for offloaded files."""
        # TODO: Integrate with proxy worker
        # For now, just log that we would generate proxies
        if self.current_manifest:
            files_count = len(self.current_manifest.files)
            self.file_label.setText(
                f"{self.file_label.text()} • Proxy generation queued ({files_count} files)"
            )
